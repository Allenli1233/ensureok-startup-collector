...
describe('check_compliance', () => {
  ...
  describe('R1 保费:仅价格语境命中', () => {
    ...
    it('金额后接每单位命中(不依赖前置线索)', () => {
      hit('年缴保费 8000。', 'R1_premium');
      hit('人均 300 每年。', 'R1_premium'); // 300 + 每年(PER_UNIT)
    });
    ...
    it('不误伤:每人/每年 引出的承保金额(复审回归)', () => {
      clean('每年累计赔偿限额500万元。');
      clean('每人赔偿限额50万元,每次事故不限人数。');
      clean('每人保额5万元。');
      clean('一年内累计赔付上限100万元。');
      clean('保费实惠,赔偿限额可达100万元。'); // 线索词不跨逗号
    });
  });

  describe('R5 绝对化承诺 + suspected 两级', () => {
    it('绝对化承诺 → R5 硬拦(含插入强调词)', () => {
      hit('本产品保证全额赔付。', 'R5_absolute');
      hit('这款稳赔不亏。', 'R5_absolute');
      hit('必赔,放心买。', 'R5_absolute');
      hit('保证足额赔付。', 'R5_absolute'); // 中间插"足额"仍命中
      hit('承诺快速理赔。', 'R5_absolute');
    });
