...
// 保费类**强**线索词(明确指价格;不含"每人/每年/全年/一年"等——那些常引出保额/限额,交给后接的 PER_UNIT 分支)
const PREMIUM_CUE = '(?:保费|费率|年缴|月缴|年费|月费|保费预算|投保预算|单价|报价|成交价|最优价|仅需|只需)';
// "金额后接的每单位"线索(五万元一年 / 300/人 / 5000元每年)
const PER_UNIT = '(?:一年|每年|全年|每月|每人|每位|/\\s*年|/\\s*人|/\\s*月)';
// R1 线索窗口**不跨逗号**(全/半角)——否则线索词会把下游"另一分句里的承保金额"拉进来(误伤)
const R1STOP = '[^。；;，、,\\n]';
...
const RULES: Array<{ rule: ComplianceViolation['rule']; re: RegExp }> = [
  // R1 保费/价格:仅在价格语境命中(货币符号/外币,或保费线索词近旁的数字)——不误伤保额/限额/免赔/人数
  {
    ...
    re: new RegExp(
      [
        ...
        `${PREMIUM_CUE}${R1STOP}{0,12}(?:${SYM}\\s*)?${AMT}`, // 保费约 8000 / 保费约五万(不跨逗号)
  ...
  // R5 绝对化承诺(保证赔/必赔/100%/稳赔)——硬拦。允许中间插入强调词(保证【足额】赔付)
  {
    rule: 'R5_absolute',
    re: /(?:保证|承诺|一定|必定|百分之?百|100\s*%|全额)[^。；;\n]{0,4}(?:赔|获赔|理赔|赔付|给付)|稳赔|必赔|包赔|保证理赔|闭眼(?:买|入)/,
  },
];
